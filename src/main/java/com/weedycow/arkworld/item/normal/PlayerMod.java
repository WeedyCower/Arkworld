...
public class PlayerMod extends NormalItem
{
    public PlayerMod()
    {
        super("player_mod", ArkCreateTab.ITEM,1);
    }
    ...
    public void onUpdate(ItemStack stack, World worldIn, Entity entityIn, int itemSlot, boolean isSelected)
    {
        ...
        if(item.getType()!=2)item.setType(2);
        if(ListUtil.getDuplicate(item.getEntries()).size()>0)
        {
            ...
            {
                if(entriesDuplicate.contains(entry) || !Arrays.stream(CapabilityItem.player).collect(Collectors.toList()).contains(entry))
                {
