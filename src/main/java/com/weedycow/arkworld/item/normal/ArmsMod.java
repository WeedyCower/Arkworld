...
public class ArmsMod extends NormalItem
{
    public ArmsMod()
    {
        super("arms_mod", ArkCreateTab.ITEM,1);
    }
    ...
    public void onUpdate(ItemStack stack, World worldIn, Entity entityIn, int itemSlot, boolean isSelected)
    {
        ...
        if(item.getType()!=0)item.setType(0);
        if(ListUtil.getDuplicate(item.getEntries()).size()>0)
        {
            ...
            {
                if(entriesDuplicate.contains(entry) || !Arrays.stream(CapabilityItem.arms).collect(Collectors.toList()).contains(entry))
                {
