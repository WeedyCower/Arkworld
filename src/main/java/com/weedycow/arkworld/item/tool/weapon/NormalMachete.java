...
public class NormalMachete extends MeleeWeapon
{
    public NormalMachete()
    {
        super(ShieldSoldier.ID,EnumWeaponRank.LOW);
        this.setUnlocalizedName(Arkworld.MODID + ".normalMachete");
        this.setRegistryName("normal_machete");
    }
    ...
    public void onUpdate(ItemStack stack, World worldIn, Entity entityIn, int itemSlot, boolean isSelected)
    {
        ...
        if(entityIn instanceof EntityPlayer && !entityIn.world.isRemote)
        {
            ...
            onDamage(weapon,melee.normalMachete.initAttackDamage,1);

            onDurability(weapon,melee.normalMachete.durability);
            ...
            if(((EntityPlayer) entityIn).getHeldItem(EnumHand.MAIN_HAND)==stack)
            {
                ...
                if(player.isSwing() && player.isAttackable() && stamina.getStamina()>=weapon.getStamina() && weapon.getLoss()<weapon.getDurability())weapon.setAnimationTick((int) (8*per));
    ...
    public boolean onEntitySwing(EntityLivingBase entityLiving, ItemStack stack)
    {
        if(entityLiving instanceof EntityPlayer)
        {
            ...
            {
                controllerAttack.setAnimation(new AnimationBuilder().addAnimation("animation.normal_machete.attack", false));
    ...
    AnimationController<NormalMachete> controllerAttack = new AnimationController<>(this, "attack", 1, this::predicate);
}
