...
public class DualSword extends MeleeWeapon
{
    public DualSword()
    {
        super(GuerrillaSiegebreaker.ID, EnumWeaponRank.NORMAL);
        this.setUnlocalizedName(Arkworld.MODID + ".dualSword");
        this.setRegistryName("dual_sword");
    }
    ...
    public void onUpdate(ItemStack stack, World worldIn, Entity entityIn, int itemSlot, boolean isSelected)
    {
        ...
        if(entityIn instanceof EntityPlayer && !entityIn.world.isRemote)
        {
            ...
            onStamina(weapon,melee.dualSword.stamina);

            onDamage(weapon,melee.dualSword.initAttackDamage,melee.dualSword.surplusAttackDamage);

            onDurability(weapon,melee.dualSword.durability);
            ...
            if(((EntityPlayer) entityIn).getHeldItem(EnumHand.MAIN_HAND)==stack)
            {
                ...
                if(player.isSwing() && player.isAttackable() && stamina.getStamina()>=weapon.getStamina() && weapon.getLoss()<weapon.getDurability())weapon.setAnimationTick((int) (20*per));

                if(weapon.getEntityHit() instanceof EntityLivingBase && weapon.getAnimationTick()==(int)(12*per))
                {
    ...
    public boolean onEntitySwing(EntityLivingBase entityLiving, ItemStack stack)
    {
        if(entityLiving instanceof EntityPlayer)
        {
            ...
            {
                controllerAttack.setAnimation(new AnimationBuilder().addAnimation("animation.dual_sword.attack", false));
    ...
    AnimationController<DualSword> controllerAttack = new AnimationController<>(this, "attack", 1, this::predicate);
}
