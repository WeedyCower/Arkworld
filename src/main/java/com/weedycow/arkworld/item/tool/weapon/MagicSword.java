...
public class MagicSword extends MeleeWeapon
{
    public MagicSword()
    {
        super(SarkazBladeweaver.ID,EnumWeaponRank.HIGH);
        this.setUnlocalizedName(Arkworld.MODID + ".magicSword");
    ...
    public void onUpdate(ItemStack stack, World worldIn, Entity entityIn, int itemSlot, boolean isSelected)
    {
        ...
        if(entityIn instanceof EntityPlayer && !entityIn.world.isRemote)
        {
            ...
            onStamina(weapon,melee.magicSword.stamina);

            onMana(weapon,melee.magicSword.mana);

            onDamage(weapon,melee.magicSword.initAttackDamage,melee.magicSword.surplusAttackDamage);

            onDurability(weapon,melee.magicSword.durability);
            ...
            if(((EntityPlayer) entityIn).getHeldItem(EnumHand.MAIN_HAND)==stack)
            {
                ...
                if(player.isSwing() && player.isAttackable() && stamina.getStamina()>=weapon.getStamina() && mana.getMana()>=weapon.getMana() && weapon.getLoss()<weapon.getDurability())weapon.setAnimationTick((int) (16*per));
                ...
                if(weapon.getEntityHit() instanceof EntityLivingBase && weapon.getAnimationTick()==(int)(8*per))
                {
                    ...
                    weapon.getEntityHit().attackEntityFrom(DamageSource.causeIndirectMagicDamage(entityIn,null), (float) (weapon.getDamage() * ((EntityPlayer)entityIn).getEntityAttribute(WLMAttributes.ATTACK_DAMAGE).getAttributeValue()));
                }
            }
        }
    }
    ...
    public void addInformation(ItemStack itemstack, World world, List<String> list, ITooltipFlag flag)
    {
        ...
        String mana = I18n.format("item.arkworld.info.mana") + " : " + weapon.getStamina();
        String durability = I18n.format("item.arkworld.info.durability") + " : " + weapon.getLoss() + "/" + weapon.getDurability();
        String add = I18n.format("item.arkworld.info.magic");
    ...
    AnimationController<MagicSword> controllerAttack = new AnimationController<>(this, "attack", 1, this::predicate);
    ...
    public boolean onEntitySwing(EntityLivingBase entityLiving, ItemStack stack)
    {
        if(entityLiving instanceof EntityPlayer)
        {
            ...
            if (stamina.getStamina() >= weapon.getStamina() && weapon.getLoss() < weapon.getDurability() && mana.getMana()>=weapon.getMana() && player.isAttackable())
            {
                controllerAttack.setAnimation(new AnimationBuilder().addAnimation("animation.magic_sword.attack", false));
