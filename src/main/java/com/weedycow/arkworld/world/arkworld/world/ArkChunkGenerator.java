...
import com.weedycow.arkworld.world.arkworld.biome.BiomeChernoberg;
...
public class ArkChunkGenerator implements IChunkGenerator
{
    ...
    private IBlockState oceanBlock = BlockRegistry.IBERIA_WATER.getDefaultState();
    ...
    public ArkChunkGenerator(World worldIn, long seed, String generatorOptions)
    {
        {
            ...
            mineshaftGenerator = (MapGenMineshaft)TerrainGen.getModdedMapGen(mineshaftGenerator, net.minecraftforge.event.terraingen.InitMapGenEvent.EventType.MINESHAFT);
            scatteredFeatureGenerator = (MapGenScatteredFeature)TerrainGen.getModdedMapGen(scatteredFeatureGenerator, net.minecraftforge.event.terraingen.InitMapGenEvent.EventType.SCATTERED_FEATURE);
    ...
    public Chunk generateChunk(int x, int z)
    {
        int p = x * 16;
        int q = z * 16;
        BlockPos blockpos = new BlockPos(p, 0, q);
        Biome biome = this.world.getBiome(blockpos.add(16, 0, 16));
        ChunkPrimer chunkprimer = new ChunkPrimer();
        this.biomesForGeneration = this.world.getBiomeProvider().getBiomes(this.biomesForGeneration, x * 16, z * 16, 16, 16);

        if(biome instanceof BiomeChernoberg)
        {
            for (int i = 0; i < settings.seaLevel+8; ++i)
            {
...
//                this.woodlandMansionGenerator.generate(this.world, x, z, chunkprimer);
    ...
    private void generateHeightmap(int x, int p_185978_2_, int z)
    {
        ...
        this.mainNoiseRegion = this.mainPerlinNoise.generateNoiseOctaves(this.mainNoiseRegion, x, p_185978_2_, z, 5, 33, 5, (double)(f / this.settings.mainNoiseScaleX), (double)(f1 / this.settings.mainNoiseScaleY), (double)(f / this.settings.mainNoiseScaleZ));
        this.minLimitRegion = this.minLimitPerlinNoise.generateNoiseOctaves(this.minLimitRegion, x, p_185978_2_, z, 5, 33, 5, (double)f, (double)f1, (double)f);
        this.maxLimitRegion = this.maxLimitPerlinNoise.generateNoiseOctaves(this.maxLimitRegion, x, p_185978_2_, z, 5, 33, 5, (double)f, (double)f1, (double)f);
        int i = 0;
        int j = 0;
...
//                this.woodlandMansionGenerator.generateStructure(this.world, this.rand, chunkpos);
        ...
        if (biome == BiomesRegistry.WASTELAND && this.settings.useWaterLakes && !flag && this.rand.nextInt(this.settings.waterLakeChance) == 0)
        {
            if (TerrainGen.populate(this, this.world, this.rand, x, z, flag, net.minecraftforge.event.terraingen.PopulateChunkEvent.Populate.EventType.LAKE))
            {
                int i1 = this.rand.nextInt(16) + 8;
                int j1 = this.rand.nextInt(256);
                int k1 = this.rand.nextInt(16) + 8;
                (new WorldGenLakes(BlockRegistry.ORIGINIUM_CONTAMINATED_WATER)).generate(this.world, this.rand, blockpos.add(i1, j1, k1));
            }
        }

//            if (biome != BiomesRegistry.WASTELAND && biome != Biomes.DESERT && biome != Biomes.DESERT_HILLS && this.settings.useWaterLakes && !flag && this.rand.nextInt(this.settings.waterLakeChance) == 0)
//            {
//                if (TerrainGen.populate(this, this.world, this.rand, x, z, flag, net.minecraftforge.event.terraingen.PopulateChunkEvent.Populate.EventType.LAKE))
//                {
//                    int i1 = this.rand.nextInt(16) + 8;
...
//            if (TerrainGen.populate(this, this.world, this.rand, x, z, flag, net.minecraftforge.event.terraingen.PopulateChunkEvent.Populate.EventType.LAVA))
//            {
...
//            }
        ...
        if (this.settings.useDungeons)
            if (TerrainGen.populate(this, this.world, this.rand, x, z, flag, net.minecraftforge.event.terraingen.PopulateChunkEvent.Populate.EventType.DUNGEON))
            {
