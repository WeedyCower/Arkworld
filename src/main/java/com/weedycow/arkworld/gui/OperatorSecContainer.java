...
public class OperatorSecContainer extends Container implements IButton
{
    Operator operator;
    EntityPlayer player;
    ...
    public OperatorSecContainer(InventoryPlayer inventoryPlayer, Operator operator, EntityPlayer player)
    {
        super();
    ...
    {
        if(buttonID==3)
            player.openGui(Arkworld.instance,1, player.world, operator.getEntityId(),0,0);
        if(buttonID==5)
            operator.setMoveMode(Operator.MoveMode.IDLE);
        if(buttonID==6)
            operator.setMoveMode(Operator.MoveMode.WANDER);
        if(buttonID==7)
            operator.setMoveMode(Operator.MoveMode.FOLLOW);
        if(buttonID==8)
            operator.setActionMode(Operator.ActionMode.NONE);
        if(buttonID==9)
            operator.setActionMode(Operator.ActionMode.FIGHTBACK);
        if(buttonID==10)
            operator.setActionMode(Operator.ActionMode.PROTECTIVE);
        if(buttonID==11)
            operator.setActionMode(Operator.ActionMode.SIGN);
        if(buttonID==12)
            operator.setActionMode(Operator.ActionMode.ENEMY);
        if(buttonID==13)
            operator.setActionMode(Operator.ActionMode.MOB);
        if(buttonID==14)
            operator.setActionMode(Operator.ActionMode.ALL);
        if(buttonID==15)
            operator.setActionMode(Operator.ActionMode.WORK);
        if(buttonID==19)
            operator.setActionMode(Operator.ActionMode.HEALING);

        if(buttonID==16 && operator.getSkillTime()==0)
        {
            operator.setSelectSkill(1);
            operator.setSkillPoint(0);
            operator.setSkillTime(0);
        }
        if(buttonID==17 && operator.getSkillTime()==0)
        {
            operator.setSelectSkill(2);
            operator.setSkillPoint(0);
            operator.setSkillTime(0);
        }
        if(buttonID==18 && operator.getSkillTime()==0)
        {
            operator.setSelectSkill(3);
            operator.setSkillPoint(0);
            operator.setSkillTime(0);
        }
        ...
        if(player.hasCapability(CapabilityRegistry.capSlotPlayer,null))
            this.stacks = new ArrayList<ItemStack>(Arrays.asList(player.getCapability(CapabilityRegistry.capSlotPlayer,
        ...
        if(buttonID==1)
        {
            ...
            if(operator.getSkillLevel()<7)
            {
                if(ListUtil.isListEqual(operator.getSluMaterial().get(operator.getSkillLevel()-1).stream().map(ItemStack::getUnlocalizedName).collect(Collectors.toList()),oriStacks.stream().map(ItemStack::getUnlocalizedName).collect(Collectors.toList())))
                {
                    ...
                    List<Integer> ns = new ArrayList<>(Arrays.asList(new Integer[] {n1,n2,n3}));
            ...
            else if(operator.getSkillRank()<3)
            {
                if(ListUtil.isListEqual(operator.getSkills().get(operator.getSelectSkill()-1).getBreakMaterial().get(operator.getSkillRank()).stream().map(ItemStack::getUnlocalizedName).collect(Collectors.toList()),oriStacks.stream().map(ItemStack::getUnlocalizedName).collect(Collectors.toList())))
                {
                    ...
                    if(min>0 && operator.getRoomType() == BlockMachine.Type.TRAINING_ROOM && operator.getTrainTime()<=0)
                    {
